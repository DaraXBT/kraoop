...
const filteredProducts = computed(() => {
  ...
  const isNew = additionalFilters.value.find(f => f.value === 'isNew')?.selected;
  ...
  return store.filterProducts({
    ...
    inStock,
    onSale,
    isNew, // Note: Store might need to support 'isNew' if it's not 'promotion' or 'date', or we assume logic
    featured,
    sortBy: sortBy.value
  });
});
