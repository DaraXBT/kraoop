...
// SEO
useSEO({
  title: "Best Selling Products - Korean Skincare & Beauty",
  description:
    "Discover our best-selling Korean skincare and beauty products. Shop customer favorites and top-rated items.",
  url: "/best-selling",
});
...
const filteredProducts = computed(() => {
  ...
  // Filter by best sellers first (products with bestSeller flag OR high rating)
  products = products.filter((p) => p.bestSeller || p.rating >= 4.5);
  ...
  activeFilters.forEach((filter) => {
    ...
    } else if (filter.value === "bestSeller") {
      // Already filtered above, but can add additional logic if needed
      products = products.filter((p) => p.bestSeller || p.rating >= 4.5);
    }
  });
...
const clearAllFilters = () => {
  ...
  additionalFilters.value.forEach((f) => {
    // Keep bestSeller selected by default
    f.selected = f.value === "bestSeller";
  });
